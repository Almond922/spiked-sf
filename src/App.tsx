...
import { Routes, Route, Navigate, Outlet, useNavigate, useLocation } from "react-router-dom"; // <<-- Added useLocation
import { useAuth } from "./AuthContext";
import { useBotId, BotIdProvider } from "./BotIdContext"; 
...
const MainLayout = () => {
    ...
    // --- Bot Connection State & Handlers (REAL LOGIC) ---
    ...
    const startBot = useCallback(async () => {
        ...
        try {
            ...
            if (newBotId && typeof newBotId === 'string' && newBotId.length > 0) {
                ...
                // ALERT: The Navbar is not shown on '/', so the user must already be aware of the Analyzer link.
    ...
    return (
        <div className="app-layout min-h-screen flex flex-col">
            ...
            {/* The NavBar is hidden on the root route ('/') and naturally hidden on the '/login' route. */}
            {shouldShowNavBar && (
                <AppHeader 
